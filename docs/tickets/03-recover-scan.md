...
尚未完成：暖機／USB 讀取途中取消、拔線重插、睡眠、失同步後復原及連續 20 次通過。已驗證兩次控制回覆仍同步時的工作期限逾時，清理後可重掃；若原始影像傳輸失敗而無法確認排空，仍只會關閉 USB 資源、回報需重插，保留權與機器狀態未確認恢復。不得宣稱失同步亦可直接重掃，也不能在忽略此錯誤後盲目重送。正式整合前需完成可驗證的重連狀態與復原策略。
