...
Blocked by：02 的文件／幾何及 03 的其他復原驗收。07 的歷史偏白調查依使用者 2026-09-26 指示暫停。Status：進行中，測試憑證套件已在開發機完成 WIA、Windows 掃描 App、更新、解除安裝及重開機實測；0.2.19 的 Windows 掃描 API 取消、重掃與閒置換孔後掃描通過，第二台電腦及傳輸中斷線復原仍待驗收。

## 2026-09-26 實體換孔驗收

使用者實際換孔後，PnP 位置比對確認接孔不同，既有 0.2.19.0 自動配對成功，沒有重新安裝或手動重啟服務。WinRT 重新發現掃描器並完成取消、彩色重掃及灰階掃描，父裝置與 MI_01 配對正常。閒置拔插換孔驗收通過，詳見 [硬體證據](../hardware.md#0219-實體換孔後掃描)。隨後閒置睡眠喚醒也以新建 WinRT 用戶端完成相同三項驗收，見 [睡眠紀錄](../hardware.md#0219-睡眠喚醒後掃描)。傳輸中拔線等其他情境由 03 追蹤，第二台電腦須另行實測。
...
- 既有 WIA 亮度、對比與實際回傳影像的映射保持正確。[07 的歷史偏白調查](07-scan-tones.md)依使用者 2026-09-26 指示暫停，不列為目前掃描交付的必要驗收。
