...
Windows 整合已具備 BMP 編碼、原生 COM 影像串流轉接及 WIA 數值設定映射。Rust 入口已完成灰階／彩色實掃與 Windows 記憶體串流讀回。原生 DLL 已通過載入、建立物件及卸載測試，IStiUSD 也已驗證初始化、實機獨占鎖定、能力查詢與釋放。同一鎖定物件已完成灰階掃描、彩色取消及彩色重掃，影像輸出期間再次呼叫掃描或解鎖會回報忙碌。WIA 影像傳輸介面與服務整合仍未完成，Windows 掃描目前不能使用本核心，詳見 [整合進度](docs/tickets/05-windows-install.md)。
