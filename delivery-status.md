...
2026-09-26 後續驗收：0.2.18.0 經 WIA 同一連線連續完成 20 次整版掃描（RGB600×10、RGB300×5、Gray600×5），沒有重試或重啟服務，外部取樣未見記憶體持續增長。0.2.19.0 已修正 START 成功後首塊前取消造成下次掃描卡住，精確階段及跨執行緒取消／重掃實機通過，已更新本機。實體換孔與閒置睡眠喚醒後，新建 Windows 掃描 API 用戶端均通過取消、彩色重掃及灰階掃描，不需重裝或手動重啟服務。第二台電腦及傳輸中斷線等其他復原情境仍未實測。
...
- 2026-09-26 使用者要求暫停 07 的歷史偏白調查，後續集中處理掃描穩定性、取消復原、換孔與跨電腦使用。
...
| 03 | 取消與復原 | 開發者 | in_progress | 0.2.18.0 的 WIA 20 次整版傳輸與記憶體觀測通過；0.2.19 核心首塊前後及跨執行緒取消後重掃、閒置換孔與睡眠後新建用戶端掃描通過，傳輸中斷線及失同步等復原未完成 |
| 05 | Windows 掃描與安裝 | 開發者 | in_progress | 開發機套件安裝／更新／解除安裝可用，Windows 掃描 App 與 Windows 傳真和掃描已有實掃；0.2.19 Windows 掃描 API 取消及重掃、閒置換孔自動配對與實掃通過，第二台電腦待驗收，歷史偏白調查暫停 |
| 06 | 列印 | 開發者 | not_started | 尚無 |
| 07 | 掃描明暗品質 | 開發者 | deferred | 2026-09-26 依使用者指示暫停，不列為目前交付阻礙 |

## Current Blockers

- 免費路線的限制：每台要用的電腦都得先信任專案測試憑證（`install.cmd` 會自動做，但需要 UAC 同意），不能公開分發。目前只在開發機驗證，第二台乾淨電腦尚未實測。開發機閒置換孔已通過。

- 首塊前 READ metadata Busy 取消後卡住已由 0.2.19 的延後排空流程修正並實測重掃。尚未確認保留權的 RESERVE Busy 取消仍會隔離 session；取消等待期間的逾時／裝置錯誤保守要求重連，不能由清理成功推論復原。詳見 03 及硬體紀錄。

- 空平台不能完成 02 的文字與精確幾何驗收。07 的偏白與淺色細節調查已暫停，不因此阻擋其他驗收。
...
- 原始影像傳輸中斷若失去框架同步，目前僅釋放 OS 資源並回報需重插，沒有證明可直接重掃。仍須驗證暖機、傳輸中拔線、掃描途中睡眠及睡眠前持有物件的復原。20 次連續整版傳輸、閒置換孔與睡眠喚醒後新建用戶端掃描已通過，不替代其他故障復原驗收。
- 已有測試憑證簽署的 WIA 套件及開發機整合證據；跨電腦安裝仍未驗證，不能以本機成功代替。
- 既有 MIT 授權保留。若選擇移植 SANE 的 GPL 實作，須先確認具體的授權與分發方案。本次沒有移植。

## Next Verifiable Output

05 接續第二台乾淨電腦驗收；03 接續傳輸中拔線、失同步、掃描途中睡眠、睡眠前持有物件及未取得保留權時取消的復原驗收。首塊前取消修復、同 session 重掃、閒置換孔與睡眠喚醒後新建用戶端掃描已有實機證據。WDK、測試憑證、WIA 登錄與開發機安裝／更新／解除安裝已有實跑證據，不重做初期方案。硬體能力須來自實際裝置，服務 context 必須由 Windows 提供。硬體測試仍序列執行，重新列舉裝置並使用新輸出目錄。

IWiaMiniDrv 的 `drvWriteItemProperties`、`drvAnalyzeItem` 與 `drvDeleteItem` 仍回不支援；WIA 2.0 串流路徑不呼叫前者，後兩者不適用單一平台項目。STI 宣告、COM aggregation、服務帳號存取 WinUSB 及 Windows 掃描取得影像已有實測。服務管理的並行載入／卸載排程、取消通知是否派送到同一 instance 與相同裝置 ID 仍須驗收。600×800 選取區仍回傳 600×801，不可將輸出尺寸任意當成 WIA 選取範圍。跨工作隔離依 03 補完，不以介面到達時間戳記當成實體重插證據。02 的文件／幾何驗收保留，07 暫停。

## Next Ticket

[05 — Windows 掃描與安裝](docs/tickets/05-windows-install.md) 的跨電腦驗收，以及 [03 — 取消與復原](docs/tickets/03-recover-scan.md) 的傳輸中拔線等其他異常復原持續追蹤。20 次整版、首塊前取消後重掃、閒置換孔及睡眠喚醒後新建用戶端掃描已通過；02 的文件／幾何與 01 的硬體異常情境保留，07 依使用者指示暫停。
...
| 暫停歷史偏白調查 | 使用者表示 Claude 已查過光源不能調整，要求不再處理偏白；07 不列為目前交付阻礙，既有 WIA 軟體亮度／對比保留 | 2026-09-26 | 05、07 |

## Verified

2026-09-26 換孔與睡眠：實體換孔由 PnP 位置變更證實，閒置睡眠／喚醒由 Windows 系統事件證實。兩次均自動辨識 0.2.19，透過新建 WinRT 用戶端完成取消、彩色重掃及灰階掃描，BMP 實際開啟檢查為空平台，無手動服務重啟或重插復原。fmt、Clippy、全部一般測試及 release 建置通過；本輪只更新驗收文件，未修改驅動。詳見 [換孔](docs/hardware.md#0219-實體換孔後掃描) 與 [睡眠喚醒](docs/hardware.md#0219-睡眠喚醒後掃描)。

2026-09-26 取消修復：250 個 Rust 測試、fmt、Clippy、release、capture_scan 範例 4 個測試及建置、release DLL 動態載入通過。原本失敗的 metadata_busy 與跨執行緒提早取消均實機通過，第一塊後取消對照也通過，皆可重掃。0.2.19.0 已完成本機更新，父裝置及 MI_01 維持原配對；更新後真正 Windows 掃描 API 的取消、彩色重掃與灰階對照成功。詳見 [05 更新與 API 驗收](docs/tickets/05-windows-install.md#2026-09-26-取消修復與更新02190) 及 [硬體紀錄](docs/hardware.md#0219-核心取消修復驗收)。20 次整版穩定性證據屬於 0.2.18.0；0.2.19.0 沒有另重跑 20 次。第二台電腦仍待實測。
