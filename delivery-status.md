...
- WIA 登錄需要簽署 catalog：本機沒有 WDK 簽署工具、未開啟 testsigning。開發機測試簽署（安裝 WDK、自簽測試憑證、`bcdedit /set testsigning on`、重開機）或正式 attestation 簽署都需要使用者決定並授權，見 [安裝方案](driver/README.md#wia-登錄方案尚未執行待授權)。在此之前 Windows 掃描整合無法實測。
...
兩條可平行工作：05 的 WIA INF 設計稿與備份／復原方案已寫成，等待使用者決定開發機測試簽署或正式簽署後，依 driver/README.md 的順序執行登錄與 Windows 掃描實測；03 依精確階段取消日誌查明首塊前取消的裝置時序，再修正及重跑失敗驗收。屬性初始化、相依驗證、讀取通知、錯誤字串、格式列舉、能力列舉、同步命令與取消事件入口已接上，不重做。硬體能力須來自實際裝置，服務 context 必須由 Windows 提供，不可用假指標替代。硬體測試仍序列執行，重新列舉裝置並使用新輸出目錄。
