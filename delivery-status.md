...
2026-09-26 後續驗收：0.2.18.0 經 WIA 同一連線連續完成 20 次整版掃描（RGB600×10、RGB300×5、Gray600×5），沒有重試或重啟服務，外部取樣未見記憶體持續增長。0.2.19.0 已修正 START 成功後首塊前取消造成下次掃描卡住，精確階段及跨執行緒取消／重掃實機通過，已更新本機。換孔及第二台電腦仍未實測。
...
| 03 | 取消與復原 | 開發者 | in_progress | 0.2.18.0 的 WIA 20 次整版傳輸與記憶體觀測通過；0.2.19 核心首塊前後及跨執行緒取消後重掃通過，斷線、睡眠及失同步復原未完成 |
| 05 | Windows 掃描與安裝 | 開發者 | in_progress | 開發機套件安裝／更新／解除安裝可用，Windows 掃描 App 與 Windows 傳真和掃描已有實掃；0.2.19 Windows 掃描 API 取消及重掃通過，換孔、拔插、第二台電腦待驗收，歷史偏白調查暫停 |
...
- 原始影像傳輸中斷若失去框架同步，目前僅釋放 OS 資源並回報需重插，沒有證明可直接重掃。正式整合前須完成重連狀態與異常復原，並驗證暖機、拔線及睡眠。20 次連續整版傳輸已通過，不替代故障復原驗收。
- 已有測試憑證簽署的 WIA 套件及開發機整合證據；跨電腦安裝、換孔與拔插仍未驗證，不能以本機成功代替。
- 既有 MIT 授權保留。若選擇移植 SANE 的 GPL 實作，須先確認具體的授權與分發方案。本次沒有移植。

## Next Verifiable Output

05 接續第二台乾淨電腦、換孔與拔插驗收；03 保留睡眠、傳輸失同步及未取得保留權時取消的復原驗收。首塊前取消修復與同 session 重掃已有實機證據。WDK、測試憑證、WIA 登錄與開發機安裝／更新／解除安裝已有實跑證據，不重做初期方案。硬體能力須來自實際裝置，服務 context 必須由 Windows 提供。硬體測試仍序列執行，重新列舉裝置並使用新輸出目錄。
...
[05 — Windows 掃描與安裝](docs/tickets/05-windows-install.md) 的跨電腦與拔插驗收，以及 [03 — 取消與復原](docs/tickets/03-recover-scan.md) 的其他異常復原持續追蹤。20 次整版及首塊前取消後重掃已通過；02 的文件／幾何與 01 的硬體異常情境保留，07 依使用者指示暫停。
