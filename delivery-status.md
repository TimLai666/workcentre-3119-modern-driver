...
2026-09-14：IWiaMiniDrv 已接上根／平台屬性初始化，以同一鎖定連線取得實機能力；解鎖失敗會隔離連線。屬性相依更新與服務整合仍待完成，Windows 掃描尚不能使用本驅動。提早取消可返回取消結果，但首塊前取消後立即重掃仍在 RESERVE Busy 120 秒後失敗，同一 USB session 對照也失敗。影像回呼取消及重掃的既有回歸通過。
...
| 05 | Windows 掃描與安裝 | 開發者 | in_progress | 原生項目樹、鎖定／串流、取消事件、BMP 格式列舉及屬性初始化已實作；相依驗證、服務與可攜套件未完成 |
