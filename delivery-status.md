...
2026-09-14：IWiaMiniDrv 已接上根／平台屬性初始化與相依驗證，解析度、模式／色深、位置／範圍及輸出尺寸會共同驗證；原生屬性發佈失敗會隔離整個 COM 物件。真正 WIA 服務與安裝整合仍未完成，Windows 掃描尚不能使用本驅動。精確階段取消對照已加入測試建置，第一塊影像後取消並以同一 USB session 重掃通過；首塊前取消的復原缺口仍由 03 追蹤。
...
| 05 | Windows 掃描與安裝 | 開發者 | in_progress | 原生項目樹、鎖定／串流、取消事件、BMP 格式列舉及屬性初始化／相依驗證已實作；服務驗收與可攜套件未完成 |
