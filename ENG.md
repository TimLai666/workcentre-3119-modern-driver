...
裝置診斷、Rust 掃描／USB 串流及部分 WIA minidriver 介面已實作，WIA 服務整合與列印仍待完成。USB 回覆與呼叫端參數均需檢查長度及數量。驅動負責回傳影像資料、能力、進度及錯誤，呼叫端軟體負責預覽畫面、編輯及檔案儲存。診斷預設不記錄序號及影像內容。
...
START 已成功、首塊影像尚未交付時，主機取消會延後到首塊 READ metadata 可讀，沿用原工作期限取得長度並排空該塊，再送 ABORT／RELEASE。這段期間不交付影像，也不重設期限。取消已提出但首塊 metadata 等待以錯誤結束時，保留原始錯誤並要求重連，不以清理成功回覆宣稱 session 可重用。首塊前直接在 READ Busy 時中止曾造成下一次 RESERVE 持續 Busy；可讀後排空的實機對照及正式修復驗收分別記錄於 [03](docs/tickets/03-recover-scan.md)，不能把測試診斷等同已安裝版本的行為。
...
WIA 選取範圍 `XEXTENT/YEXTENT` 與輸出尺寸屬性用途不同。正式屬性模型須維護範圍、位置、解析度與頁面間的關係，不能因 READ 回傳不同就假定應覆寫選取範圍。Microsoft 建議應用程式以影像標頭取得實際尺寸。`wia::scan_bmp` 本身只接收數值快照；COM minidriver 已提供初始屬性建立及 SDK 發佈入口，相依同步、服務整合與幾何契約仍由 05 驗收。[Microsoft XEXTENT](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/wia-ips-xextent)、[PIXELS_PER_LINE](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/wia-ipa-pixels-per-line)
...
原生回呼以獨立宣告 ABI 的測試物件供應 Windows HGLOBAL IStream，核心流程測試使用明示的合成影像與清理結果。實機測試則沿用同一鎖定物件跑灰階、彩色取消及重掃，驗收矩陣見 [05](docs/tickets/05-windows-install.md)。`IWiaMiniDrv::drvAcquireItemData` 已重用此入口，不能用任意指標偽造 WIA 服務的 property context。原生項目樹、生命週期及屬性相依更新已實作，服務提供的串流尚待整合驗收。進度回呼目前只在開始、影像塊完成與結束時執行；WIA_EVENT_CANCEL_IO 已能通知目前工作，不依賴下一次進度回呼，但提早取消後重掃尚未通過實機驗收。
...
`drvAcquireItemData` 只接受 WIA2 DOWNLOAD，先檢查 SDK 144-byte transfer context 與 callback，再從真實 WIA context 讀取設定、格式及項目名稱。原生讀取只接受 S_OK，BSTR 有 16K UTF-16 上限且由 RAII 釋放。根／非影像／不可傳輸項目拒絕，設定沿用 `FlatbedSettings::to_request`；能力仍由掃描使用的同一 session 驗證。傳輸不自行重新鎖定 USB，沿用 `State::transfer_bmp`。Completed／單張平台開始前 Skipped 回 S_OK，確認取消才回 S_FALSE；串流錯誤不因 Interrupted 被誤判取消，原生錯誤 HRESULT 保留。[原生 acquire](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wiamindr_lh/nf-wiamindr_lh-iwiaminidrv-drvacquireitemdata)、[WIA2 傳輸](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/istream-data-transfer-driver-changes)、[屬性讀取](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/reading-wia-item-properties-by-a-driver)

`cancel.rs` 在讀取屬性前建立每次工作的取消旗標。`drvNotifyPnpEvent` 核對 WIA_EVENT_CANCEL_IO、裝置 BSTR 與 reserved=0，只通知相符的作用中工作，不取得生命週期鎖或在通知執行緒呼叫 USB／COM。完成與取消由同一個短暫 Mutex 排序，舊工作旗標不留給下一次掃描。掃描核心以 `HostCancelled` 標記真正的主機取消，`DiagnosticError` 保留起因；只有確認清理成功且沒有輸出錯誤時才回取消。裝置自行回報 cancelled、一般 Interrupted 與清理失敗不能靠目前旗標冒充成功取消。事件後仍需等待正在執行的有限 USB 呼叫或排空，沒有新增無限重試。[取消事件契約](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wiamindr_lh/nf-wiamindr_lh-iwiaminidrv-drvnotifypnpevent)

`formats.rs` 在生命週期借用期間透過 `wiasGetItemType` 讀取真實項目型別，只為可傳輸的影像項目回傳一個 BMP／TYMED_FILE。格式表為程序生命週期的不可變靜態資料，服務不負責釋放；根、資料夾與無資料項目拒絕。原生 WIA context 的實際列舉仍須在服務整合階段驗證。[格式列舉契約](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wiamindr_lh/nf-wiamindr_lh-iwiaminidrv-drvgetwiaformatinfo)
...
`drvInitItemProperties` 透過保留的 IStiDevice 鎖定裝置，在同一 STI session 執行既有 INQUIRY，成功解鎖後才建立服務屬性。整段流程維持同一個 WIA 連線借用，原生呼叫期間不持有狀態 Mutex；重入的中斷連線／其他工作立即返回 Busy。查詢錯誤仍解鎖，原始錯誤優先於清理錯誤。服務解鎖失敗後隔離此 WIA 連線，不自動重送解鎖。[Microsoft LockDevice](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/sti/nf-sti-istidevice-lockdevice)、[UnLockDevice](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/sti/nf-sti-istidevice-unlockdevice)

`properties/catalog.rs` 區分根／平台屬性，以實際能力與現有核心的交集建立解析度、模式及範圍。初始採最小支援解析度、優先灰階、全平台範圍；色深及 Y 解析度的有效清單對應當下模式與 X 解析度。位置依協定百分之一英吋步進，明暗維持中性 0。光學資訊依 Xerox 本型號規格為 600 × 2400 dpi，與可選解析度分開；不因此開放尚未查明的非對稱掃描。BMP 大小先依目前選取區、列填補、標頭與灰階色盤計算，不設為零。裝置實際回傳多一列的幾何契約仍待 02／05 驗收，正式傳輸須同步實際影像資訊。[Xerox 規格](https://www.office.xerox.com/latest/W31BR-01.PDF)、[WIA 尺寸契約](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/wia-ipa-item-size)

`properties/native.rs` 依名稱 → 初始值 → 有效值／權限的順序寫入，任一步非 S_OK 立即失敗，未知正值不當成功。屬性名稱沿用服務建立的項目名稱，ICM profile 交由服務依安裝資訊維護，不覆寫空值。動態影像屬性不標記可快取。一般測試使用共用寫入流程的合成接收端，不建立假 WIA context；真正 SDK 寫入、相依更新、即時狀態及服務消費仍須整合驗收。根狀態目前 0，不把 INQUIRY 成功當成平台已就緒。[屬性初始化順序](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/adding-wia-properties-to-a-wia-item)、[共通屬性所有權](https://learn.microsoft.com/en-us/previous-versions/windows/desktop/wia/-wia-wiaitempropcommonitem)
...
`properties/validation.rs` 先在 Rust 內處理模式、色深、解析度及選取區，再交給 `catalog.with_settings` 驗證實機能力並建立相依範圍。DATATYPE 決定 DEPTH；XRES／YRES 兩個清單都列出全部解析度（WinRT 以初始化時快取的清單在客戶端驗證），只寫其中一軸時另一軸跟隨，兩軸同時寫入且不同才拒絕。只寫不相容的 DEPTH 會拒絕，不反向改變模式。解析度改變時只縮放及量化未明確寫入的座標；明確寫入的起點會貼齊最近的 1/100 英寸硬體步進（Windows 用戶端以英寸換算再四捨五入成像素，常落在步進之外，例如 Windows 掃描 App 在 600 dpi 寫 XPOS=7），貼齊後若明確範圍放不下就退到下一個步進，仍放不下才拒絕。範圍本身不調整；只有未寫入的相依欄位可調整。這是設定的像素座標轉換，不是影像縮放或明暗處理。[相依解析度](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/wia-ips-xres)、[位置](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/wia-ips-xpos)
...
`com_server::sti` 的 19 個方法位置依 SDK `stiusd.h` 宣告。Initialize 驗證 Unicode STI 2 版本，保留 helper 參考並取得有上限的 UTF-16 port name，借用的登錄句柄不使用也不關閉。helper 呼叫在狀態鎖外執行，失敗初始化可重試。Initialize 成功後即嘗試開啟 USB（port name 為 `AUTO` 時以專案 GUID 列舉唯一 MI_00，否則只開與 helper 路徑相符的裝置），LockDevice 在尚未開啟時重試；WinUSB 每台裝置只允許一個開啟中的句柄，裝置節點暴露 GUID_DEVINTERFACE_IMAGE 後 WIA 服務會在我們關閉時搶佔通知句柄，因此 USB 句柄從第一次開啟保留到最終 Release，UnLockDevice 只清除 STI 鎖旗標。Diagnostic 僅在鎖定後執行既有 INQUIRY 並驗證能力回覆，不能用此結果宣稱掃描馬達已就緒。未實作的狀態、reset、raw、escape 與通知回傳不支援，GetCapabilities 目前不宣告 WIA／通知能力。[Microsoft IStiUSD](https://learn.microsoft.com/en-us/windows-hardware/drivers/image/providing-an-istiusd-interface)
...
已實作的流程為「IStiUSD 鎖定 → 暫借同一 session → 掃描／BMP 輸出 → 依核心回報歸還或隔離」。借用期間不持有狀態 Mutex，讓影像呼叫端重入查詢時能返回；再次掃描、解鎖及診斷則立即回報忙碌。成功、尚未嘗試 RESERVE 的取消、或經確認清理完成的失敗可以歸還連線。有效 INQUIRY 後、RESERVE 前取消也屬於可歸還情況。命令嘗試旗標在 USB 寫入前設立，跨 Busy 等待保留，避免把已送 RESERVE 後的取消誤當成未接觸裝置。失同步、未知消耗量及清理失敗必須隔離，不能分析錯誤字串來判斷可重用。隔離後本物件不再開啟或送命令，重新連線的跨物件身分驗證仍由 03 完成。
