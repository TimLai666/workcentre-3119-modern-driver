...
每塊最多 16 MiB、每工作最多 256 MiB／4096 塊，讀取緩衝區 64 KiB。工作期限 120 秒，另加正在執行的有限 USB 呼叫與最多兩個清理命令；呼叫端 callback 必須及時返回。取消在已知資料塊排空後送 ABORT／RELEASE，避免把未讀影像當成命令回覆。USB 中途失敗、短寫或失去框架同步時不盲送復原命令，回報需重插；這種情況不能宣稱裝置已復原。呼叫端錯誤與可展開堆疊的 panic 會取消並釋放，合成測試已覆蓋。強制終止行程、panic=abort 及斷電仍無法依靠此清理路徑。
