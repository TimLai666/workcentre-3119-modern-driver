...
- `src/com_server/minidrv.rs` 提供 IWiaMiniDrv 身分與多用戶端生命週期，`minidrv/tree.rs` 使用 Windows 原生根／平台項目。`locking.rs` 經服務的 IStiDevice 鎖定／解鎖；`properties.rs` 讀取真正服務 context，`acquire.rs` 接上同一 STI 連線的串流傳輸，`cancel.rs` 通知相符的作用中工作，`formats.rs` 列舉 BMP。更動後跑 `cargo test --offline --lib com_server::minidrv::`、COM 介面測試及當次 release DLL 動態測試。一般測試不操作 USB、登錄系統或偽造 WIA context。屬性初始化、相依驗證與服務整合仍未完成。項目樹 getter 的參考所有權須逐 API 核對，不能一律套用 QI 的 owned 規則。
