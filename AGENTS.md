...
- `src/com_server/minidrv.rs` 提供 IWiaMiniDrv 身分與多用戶端生命週期，`minidrv/tree.rs` 使用 Windows 原生根／平台項目。`locking.rs` 經服務的 IStiDevice 鎖定／解鎖；`properties.rs` 讀取真正服務 context，`acquire.rs` 接上同一 STI 連線的串流傳輸。更動後跑 `cargo test --offline --lib com_server::minidrv::`、COM 介面測試及當次 release DLL 動態測試。一般測試不操作 USB、登錄系統或偽造 WIA context。屬性初始化、相依驗證、取消事件與服務整合仍未完成。項目樹 getter 的參考所有權須逐 API 核對，不能一律套用 QI 的 owned 規則。
- `minidrv/locking/hardware.rs` 的 `actual_wia_lock_routes_through_sti_and_releases` 只鎖定／INQUIRY；`actual_wia_dispatch_scans_cancels_and_rescans` 另啟動灰階、彩色取消及重掃。必須重新列舉 `WC3119_TEST_STI_PATH`，掃描另指定不存在的 `WC3119_TEST_OUTPUT_DIR`，兩者逐一以 `--ignored --exact` 執行。服務 helper、屬性快照與 callback 為合成測試資料，USB、Windows 項目與 IStream 為實際資源，不能當成 Windows 掃描驗收。
