...
- `src/com_server/minidrv.rs` 提供 IWiaMiniDrv 身分與多用戶端生命週期，`minidrv/tree.rs` 使用 Windows 原生根／平台項目。更動後跑 `cargo test --offline --lib com_server::minidrv::tests`、COM 介面測試及當次 release DLL 動態測試。原生項目測試使用合成名稱及真正 Windows 物件，不操作 USB、登錄系統或偽造 WIA service context；屬性及 drvAcquireItemData 尚未完成。項目樹 getter 的參考所有權須逐 API 核對，不能一律套用 QI 的 owned 規則。
